...
function Box({ open, logoPath, showLinks, shiver, showConfetti, showLabels }: BoxProps) {
  ...
  const frontLogoTexture = useTexture('/HuloolAlwaraq_Logo.png')
  useFrame(() => {
    if (groupRef.current) {
      if (shiver) {
    ...
    <div ref={containerRef} className="relative w-full h-full max-w-3xl mx-auto touch-pan-y" style={{maxHeight: '90vh'}}>
      ...
      >
        ...
        <Float floatIntensity={0.2} speed={1.2}>
          <Box open={open} logoPath={isArabic ? "/HuloolAlwaraq_Logo.png" : "/HuloolAlwaraq_Logo.png"} showLinks={showLinks} shiver={shiver} showConfetti={showConfetti} showLabels={showLabels} />
        </Float>
